...
class TimeTools {
     ...
     */
    static getTotalDaysOfMonth(date) {
        let inputDate = date ? new Date(date) : new Date();
        if (!isValidDate(inputDate)) {
            throw new Error("Invalid date");
        }
        // @ts-ignore
        let data = new Date(new Date(inputDate).getFullYear(), new Date(inputDate).getMonth() + 1, 0).getDate();
        return data;
    }
    /**
     * 判断给定的时间是否为闰年
...
export { BaseTools, TimeTools, useLocalStorage, useSessionStorage };
